...
## Historical Graph Schema 5 Migration
...
## Current Graph Schema 7

Schema 7 adds bounded persisted workspace context and normalized
manifest-dependency evidence for project-aware MCP exploration. Schema 6 graphs
are not upgraded in place because the older format cannot reconstruct manifest
declaration provenance. A mismatch therefore requires a graph rebuild:

```bash
codenib index /path/to/repository --preset graph --rebuild
```

Source-only graphs remain valid schema-7 graphs with `workspace_context=None`.
The source query surface and native `FactBatchBuffer` contract remain
unchanged; project-aware retrieval requires matching workspace metadata.
