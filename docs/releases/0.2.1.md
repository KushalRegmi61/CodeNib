...
CodeNib does not create `.mcp.json`, `AGENTS.md`, or `CLAUDE.md` by default. An
explicit `codegraph init --install-context-planner` opt-in installs the
project-local context-planner Skill and a marked `.claude/CLAUDE.md` section;
the managed files are drift-checked and can be removed with
`codegraph uninstall --remove-context-planner`. It refuses to overwrite an
unmanaged name or remove a receipt-owned registration whose command has drifted
